...
class ElGamalEncryption:
    ...
    def randomize_binary(self, public_key, encrypted_chunks):
        randomized_chunks = []
        for chunk in encrypted_chunks:
            randomized_chunk = self.randomize_ciphertext(public_key, chunk)
            randomized_chunks.append(randomized_chunk)
        return randomized_chunks


# # Example usage
# ElGamal = ElGamalEncryption(512)  # Initialize ElGamalEncryption with 512-bit prime
# private_key, public_key = ElGamal.generate_keys()  # Generate keys
# binary_message = b'Hello, World!'  # Message to encrypt

# print(f"Original message: {binary_message}\n")
# # Encrypt the binary data
# encrypted_chunks = ElGamal.encrypt_binary(public_key, binary_message)
# print(f"Encrypted: {encrypted_chunks}\n")

# # Randomize the ciphertext (optional step for demonstrating ciphertext randomization)
# randomized_ciphertexts = [ElGamal.randomize_ciphertext(public_key, c) for c in encrypted_chunks]
# print(f"Randomized ciphertext: {randomized_ciphertexts}\n")

# # Decrypt the binary data
# decrypted_binary = ElGamal.decrypt_binary(private_key, randomized_ciphertexts)
